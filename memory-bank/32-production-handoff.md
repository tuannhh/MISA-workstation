...
| O4 runtime session/secret/DB/uploads | DevOps MISA | SQL-backed session logic đã có; production bắt buộc `NODE_ENV=production` + `SESSION_SECRET` | Secret injection, Cloud SQL connection/least privilege, persistent `DATA_DIR` hoặc object storage, restart/revoke smoke. **Gemini API key + SMTP mail:** 2 cách (biến môi trường, hoặc file `data/gemini.key`/`data/smtp.json` trong `DATA_DIR`) — không có admin UI, xem `13-deployment-runbook.md` §F | HANDOFF cấu hình, không mở code task |
