...
| BR-AI-018 | BR-AI-018 | green | server/test/integration-jobs.test.js | monitor.js analyzePending() (DB-coupled: đọc/ghi hàng đợi mentions, chunk 8, chỉ ghi output hợp lệ + clamp score; queue rỗng không egress; lỗi 1 batch không hỏng batch sau) — Gemini fake, chạy SQLite + MySQL |
...
| UI-REM-001 | UI-REM-001 | green | server/test/unit-reminders-ui.test.js | W3 Reminders: CRUD payload generic chỉ allowlist field công khai (không nhận subject_id), feature-flag/route explicit; inbox R057-R059 chỉ ack theo permission server-returned và có Desktop MDS + Native composition riêng. |
| UI-MONITOR-002 | UI-MONITOR-002 | green | server/test/unit-interactions-ui.test.js | W3 Monitoring Sources: list/form Desktop+Native cho R123-R126; API allowlist name/url/type/enabled, không gửi owner/mode hay trigger scan; URL/SSRF/RSS và Module-admin-only vẫn do server quyết định. |
| UI-MONITOR-003 | UI-MONITOR-003 | green | server/test/unit-interactions-ui.test.js | W3 Monitoring Mentions Read: Desktop/Native cho R110; browser chỉ gửi page/search/category/sentiment/status đến API protected, không có scan/bulk-delete hay mutation trong slice đọc/lọc. |
| UI-MONITOR-004 | UI-MONITOR-004 | green | server/test/unit-interactions-ui.test.js | W3 Monitoring Mention Review: R111 chỉ mở cho principal có `monitoring:edit`; PUT chỉ allowlist `sentiment/status/next_action/assignee/tags/note`, không có owner/scan/delete; Desktop form và Native mini-app là hai composition MDS riêng. |
| UI-MONITOR-005 | UI-MONITOR-005 | green | server/test/unit-interactions-ui.test.js | W3 Monitoring Queries: CRUD bộ từ khóa chỉ qua endpoint protected, payload allowlist `name/category/query_type/include/exclude/enabled/grounding`, không gọi scan; Desktop và Native mini-app tách riêng. |
