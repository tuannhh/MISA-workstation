...
| F2-fixation | F2-fixation | known-red | server/test/target-session-f2.test.js | G1B.3 target: session id (cookie) phải đổi khi login lại trên cookie có sẵn — hiện KHÔNG đổi (`auth.login()` không gọi `req.session.regenerate()`). Allowlist `memory-bank/g1b-allowlist.json` {owner:backend, expiry:2026-12-31, wave:W1.7} |
| F2-ratelimit | F2-ratelimit | known-red | server/test/target-session-f2.test.js | G1B.3 target: `/api/login` phải rate-limit sau nhiều lần sai mật khẩu — hiện KHÔNG có giới hạn. Allowlist `memory-bank/g1b-allowlist.json` {owner:backend, expiry:2026-12-31, wave:W1.7} |
