...
| AI-E001..E012 (G1A.7 AI golden set) | AI-E001..AI-E012 | **PARTIAL — SQLite green (18/18), MySQL RED (3/18 fail)** | server/test/integration-ai-golden.test.js | golden happy CÓ key + fetch mock (fake/fixture, không gọi Gemini/Internet thật) cho ĐỦ 12 luồng threat-model (`06-threat-model.md` §A): E001 voice, E002 card-text, E003 card-image, E004 award-extract (2 nhánh text+url), E005 award-advice, E006 event-extract, E007 analyzeBatch, E008 groundIngest (+ fallback rỗng), E009 siteGroundIngest (qua runScan mode=site), E010 highlights, E011 competitor-brief, E012 campaign evaluate; + 4 test CHARACTERIZATION lỗi dùng chung tầng `gemini.js call()` (malformed JSON/quota HTTP 429/timeout AbortError/genImage không trả ảnh). **MySQL FAIL thật, KHÔNG phải flaky-bỏ-qua:** AI-E001 lộ **F19** (route `/ai/interaction-voice` 502 mọi lần vì `COLLATE NOCASE` không hợp lệ trên MySQL, `ai.js:26,30`); AI-E008/AI-E009 lộ **F20 nghi vấn CHƯA XÁC ĐỊNH ROOT CAUSE** (ghi `mentions` báo `added>0` nhưng SELECT lại ngay sau không thấy dòng, nghi cơ chế đồng bộ `mysql-worker.js`). Xem chi tiết `01-audit-findings.md` §F19/F20. **CHƯA closed — cần Codex fix F19 + điều tra F20 rồi chạy lại `DB_CLIENT=mysql ALLOW_TEST_DB_CREATE=1 node --test server/test/integration-ai-golden.test.js` tới khi 18/18 mới đổi status này thành `green`.** |
