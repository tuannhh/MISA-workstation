...
`router.use(requireAuth)` áp dụng chung cho toàn bộ `routes.js`/`ai.js` (`routes.js:14`, `ai.js:13`) — bắt buộc đăng nhập. Trên nền đó, **142/148 route** thêm `requirePerm(module, action)` theo đúng module+hành động (xem [`07-route-catalog.md`](07-route-catalog.md) cột `auth`) — 3 route mới batch W1.ADMIN (2026-08-31: R146/R147/R148) đều có `requirePerm`. 6 route chỉ dừng ở `requireAuth`, có lý do riêng từng route (không phải bỏ sót đồng loạt):
