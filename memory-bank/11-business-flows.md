...
3. **Chống trùng**: mỗi mốc nhắc kiểm tra `reminder_log` (unique theo `date_id`+`occur_date`+`seq`+`channel`+`recipient_user_id`) trước khi tạo — an toàn khi `runOnce()` bị gọi nhiều lần (tay + tự động) trong cùng ngày.
