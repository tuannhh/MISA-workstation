...
| R045 | POST | /api/interactions | **`requirePerm(interactions,create)` tương đương, kiểm INLINE trong handler (không còn literal ở khai báo route — D13 batch `RBAC-EXP-B3` 2026-08-30, entity Direct đầu tiên)**: router-level `requireAuth` + `policyService.prepareCreate()`/PolicyEngine, module `interactions`/action `create` — tự gán `owner_id`/`created_by` = principal hiện tại — khai tường minh ở `scripts/verify-g0.mjs#PILOT_INLINE_PERM_ROUTES` | interactions | — | không | routes.js:688 |
| R046 | GET | /api/bookings | requirePerm(partners,view) | bookings | org_fee (maskMoney ad-hoc) | không | routes.js:599 |
| R047 | POST | /api/bookings | **`requirePerm(partners,create)` tương đương, kiểm INLINE trong handler (D13 batch `RBAC-EXP-B3` 2026-08-30, entity Direct)**: router-level `requireAuth` + `policyService.prepareCreate()`/PolicyEngine, module `partners`/action `create` — tự gán `owner_id`/`created_by` = principal hiện tại — khai tường minh ở `scripts/verify-g0.mjs#PILOT_INLINE_PERM_ROUTES` | bookings | org_fee (**KHÔNG gate write** — F1) | không | routes.js:740 |
| R048 | PUT | /api/bookings/:id | **`requirePerm(partners,edit)` tương đương, kiểm INLINE trong handler (D13 batch `RBAC-EXP-B3`)**: router-level `requireAuth` + `policyService.assertWritable()`/PolicyEngine, module `partners`/action `edit` — executor chỉ qua khi `owner_id` = chính họ (Direct entity, khác Global) — khai tường minh ở `scripts/verify-g0.mjs#PILOT_INLINE_PERM_ROUTES` | bookings | org_fee (F1) | không | routes.js:752 |
| R049 | DELETE | /api/bookings/:id | **`requirePerm(partners,delete)` tương đương, kiểm INLINE trong handler (D13 batch `RBAC-EXP-B3`)**: router-level `requireAuth` + `policyService.assertWritable()`/PolicyEngine (chỉ Admin/Super Admin qua — Direct entity, executor không bao giờ xoá kể cả chủ sở hữu), module `partners`/action `delete` — khai tường minh ở `scripts/verify-g0.mjs#PILOT_INLINE_PERM_ROUTES` | bookings | — | không | routes.js:764 |
...
| R062 | GET | /api/awards | requirePerm(awards,view) | awards | cost (Confidential — che qua `policyService.projectRecord()`/PolicyEngine, D13 batch `RBAC-EXP-B4`, không còn maskMoney ad-hoc) | không | routes.js:1039 |
| R063 | GET | /api/awards/:id | requirePerm(awards,view) | awards+award_participations+attachments | cost/budget (Confidential — `projectRecord()`, D13 batch `RBAC-EXP-B4`) | không (metadata) | routes.js:1060 |
| R064 | POST | /api/awards | **`requirePerm(awards,create)` tương đương, kiểm INLINE trong handler (D13 batch `RBAC-EXP-B4` 2026-08-30, entity Direct)**: router-level `requireAuth` + `policyService.prepareCreate()`/PolicyEngine, module `awards`/action `create` — tự gán `owner_id`/`created_by` = principal hiện tại — khai tường minh ở `scripts/verify-g0.mjs#PILOT_INLINE_PERM_ROUTES` | awards | cost (không còn "KHÔNG gate write" — F1 đã đóng cho entity này) | không | routes.js:1071 |
| R065 | PUT | /api/awards/:id | **`requirePerm(awards,edit)` tương đương, kiểm INLINE trong handler (D13 batch `RBAC-EXP-B4`)**: router-level `requireAuth` + `policyService.assertWritable()`/PolicyEngine, module `awards`/action `edit` — executor chỉ qua khi `owner_id` = chính họ (Direct entity) — khai tường minh ở `scripts/verify-g0.mjs#PILOT_INLINE_PERM_ROUTES` | awards | cost (F1 đã đóng) | không | routes.js:1084 |
| R066 | DELETE | /api/awards/:id | **`requirePerm(awards,delete)` tương đương, kiểm INLINE trong handler (D13 batch `RBAC-EXP-B4`)**: router-level `requireAuth` + `policyService.assertWritable()`/PolicyEngine (chỉ Admin/Super Admin qua — Direct entity, executor không bao giờ xoá kể cả chủ sở hữu), module `awards`/action `delete` — khai tường minh ở `scripts/verify-g0.mjs#PILOT_INLINE_PERM_ROUTES` | awards+attachments | — | có (xoá file vật lý) | routes.js:1097 |
| R067 | POST | /api/awards/:id/participations | **`requirePerm(awards,create)` tương đương, kiểm INLINE trong handler (D13 batch `RBAC-EXP-B4`)**: router-level `requireAuth` + `policyService.prepareCreate()`/PolicyEngine, module `awards`/action `create` (entity `award_participation` — Direct riêng, KHÔNG kế thừa owner của award cha) — tự gán `owner_id`/`created_by` — khai tường minh ở `scripts/verify-g0.mjs#PILOT_INLINE_PERM_ROUTES` | award_participations | budget (F1 đã đóng) | không | routes.js:1114 |
| R068 | PUT | /api/awards/:id/participations/:pid | **`requirePerm(awards,edit)` tương đương, kiểm INLINE trong handler (D13 batch `RBAC-EXP-B4`)**: router-level `requireAuth` + `policyService.assertWritable()`/PolicyEngine, module `awards`/action `edit` — executor chỉ qua khi `owner_id` = chính họ — khai tường minh ở `scripts/verify-g0.mjs#PILOT_INLINE_PERM_ROUTES` | award_participations | budget (F1 đã đóng) | không | routes.js:1127 |
| R069 | DELETE | /api/awards/:id/participations/:pid | **`requirePerm(awards,delete)` tương đương, kiểm INLINE trong handler (D13 batch `RBAC-EXP-B4` — SỬA ĐÚNG: trước đây map nhầm vào quyền `edit` nên executor xoá được, nay chỉ Admin/Super Admin)**: router-level `requireAuth` + `policyService.assertWritable()`/PolicyEngine, module `awards`/action `delete` — khai tường minh ở `scripts/verify-g0.mjs#PILOT_INLINE_PERM_ROUTES` | award_participations | — | không | routes.js:1138 |
...
| R074 | GET | /api/suppliers/:id | requirePerm(suppliers,view) | suppliers+supplier_quotes+supplier_transactions+attachments+supplier_contacts | unit_price/value (Confidential — `projectRecord()`/PolicyEngine, D13 batch `RBAC-EXP-B5`, không còn maskMoney ad-hoc) | không (metadata) | routes.js:1196 |
| R075 | POST | /api/suppliers/:id/contacts | **`requirePerm(suppliers,create)` tương đương, kiểm INLINE trong handler (D13 batch `RBAC-EXP-B5` 2026-08-30, entity Direct)**: router-level `requireAuth` + `policyService.prepareCreate()`/PolicyEngine, module `suppliers`/action `create` — tự gán `owner_id`/`created_by` = principal hiện tại — khai tường minh ở `scripts/verify-g0.mjs#PILOT_INLINE_PERM_ROUTES` | supplier_contacts | — | không | routes.js:1214 |
| R076 | PUT | /api/suppliers/:id/contacts/:cid | **`requirePerm(suppliers,edit)` tương đương, kiểm INLINE trong handler (D13 batch `RBAC-EXP-B5`)**: router-level `requireAuth` + `policyService.assertWritable()`/PolicyEngine, module `suppliers`/action `edit` — executor chỉ qua khi `owner_id` = chính họ — khai tường minh ở `scripts/verify-g0.mjs#PILOT_INLINE_PERM_ROUTES` | supplier_contacts | — | không | routes.js:1227 |
| R077 | DELETE | /api/suppliers/:id/contacts/:cid | **`requirePerm(suppliers,delete)` tương đương, kiểm INLINE trong handler (D13 batch `RBAC-EXP-B5` — SỬA ĐÚNG: trước đây map nhầm vào quyền `edit`, nay chỉ Admin/Super Admin)**: router-level `requireAuth` + `policyService.assertWritable()`/PolicyEngine, module `suppliers`/action `delete` — khai tường minh ở `scripts/verify-g0.mjs#PILOT_INLINE_PERM_ROUTES` | supplier_contacts | — | không | routes.js:1239 |
| R078 | POST | /api/suppliers/:id/transactions | **`requirePerm(suppliers,create)` tương đương, kiểm INLINE trong handler (D13 batch `RBAC-EXP-B5`, entity Direct)**: router-level `requireAuth` + `policyService.prepareCreate()`/PolicyEngine, module `suppliers`/action `create` — khai tường minh ở `scripts/verify-g0.mjs#PILOT_INLINE_PERM_ROUTES` | supplier_transactions | value (không còn "KHÔNG gate write" — F1 đã đóng cho entity này) | không | routes.js:1252 |
| R079 | PUT | /api/suppliers/:id/transactions/:tid | **`requirePerm(suppliers,edit)` tương đương, kiểm INLINE trong handler (D13 batch `RBAC-EXP-B5`)**: router-level `requireAuth` + `policyService.assertWritable()`/PolicyEngine, module `suppliers`/action `edit` — executor chỉ qua khi `owner_id` = chính họ — khai tường minh ở `scripts/verify-g0.mjs#PILOT_INLINE_PERM_ROUTES` | supplier_transactions | value (F1 đã đóng) | không | routes.js:1265 |
| R080 | DELETE | /api/suppliers/:id/transactions/:tid | **`requirePerm(suppliers,delete)` tương đương, kiểm INLINE trong handler (D13 batch `RBAC-EXP-B5` — SỬA ĐÚNG: trước đây map nhầm vào quyền `edit`, nay chỉ Admin/Super Admin)**: router-level `requireAuth` + `policyService.assertWritable()`/PolicyEngine, module `suppliers`/action `delete` — khai tường minh ở `scripts/verify-g0.mjs#PILOT_INLINE_PERM_ROUTES` | supplier_transactions | — | không | routes.js:1277 |
...
| R084 | POST | /api/suppliers/:id/quotes | **`requirePerm(suppliers,create)` tương đương, kiểm INLINE trong handler (D13 batch `RBAC-EXP-B5`, entity Direct)**: router-level `requireAuth` + `policyService.prepareCreate()`/PolicyEngine, module `suppliers`/action `create` — khai tường minh ở `scripts/verify-g0.mjs#PILOT_INLINE_PERM_ROUTES` | supplier_quotes | unit_price (không còn "KHÔNG gate write" — F1 đã đóng cho entity này) | không | routes.js:1317 |
| R085 | DELETE | /api/suppliers/:id/quotes/:qid | **`requirePerm(suppliers,delete)` tương đương, kiểm INLINE trong handler (D13 batch `RBAC-EXP-B5` — SỬA ĐÚNG: trước đây map nhầm vào quyền `edit`, nay chỉ Admin/Super Admin)**: router-level `requireAuth` + `policyService.assertWritable()`/PolicyEngine, module `suppliers`/action `delete` — khai tường minh ở `scripts/verify-g0.mjs#PILOT_INLINE_PERM_ROUTES` | supplier_quotes | — | không | routes.js:1328 |
...
| R087 | GET | /api/events | requirePerm(events,view) | events+event_costs | total_cost (event_cost.amount Confidential — che qua `policy.canReadField()` theo owner_id CHA, D13 batch `RBAC-EXP-B4`, không còn maskMoney ad-hoc) | không | routes.js:1298 |
| R088 | GET | /api/events/:id | requirePerm(events,view) | events+event_costs+attachments | costs/totals (event_cost.amount Confidential — `policy.canReadField()` theo owner_id CHA, D13 batch `RBAC-EXP-B4`) | không (metadata) | routes.js:1321 |
| R089 | POST | /api/events | **`requirePerm(events,create)` tương đương, kiểm INLINE trong handler (D13 batch `RBAC-EXP-B4` 2026-08-30, entity Direct)**: router-level `requireAuth` + `policyService.prepareCreate()`/PolicyEngine, module `events`/action `create` — tự gán `owner_id`/`created_by` = principal hiện tại — khai tường minh ở `scripts/verify-g0.mjs#PILOT_INLINE_PERM_ROUTES` | events | — | không | routes.js:1339 |
| R090 | PUT | /api/events/:id | **`requirePerm(events,edit)` tương đương, kiểm INLINE trong handler (D13 batch `RBAC-EXP-B4`)**: router-level `requireAuth` + `policyService.assertWritable()`/PolicyEngine, module `events`/action `edit` — executor chỉ qua khi `owner_id` = chính họ (Direct entity) — khai tường minh ở `scripts/verify-g0.mjs#PILOT_INLINE_PERM_ROUTES` | events | — | không | routes.js:1352 |
| R091 | DELETE | /api/events/:id | **`requirePerm(events,delete)` tương đương, kiểm INLINE trong handler (D13 batch `RBAC-EXP-B4`)**: router-level `requireAuth` + `policyService.assertWritable()`/PolicyEngine (chỉ Admin/Super Admin qua — Direct entity, executor không bao giờ xoá kể cả chủ sở hữu), module `events`/action `delete` — khai tường minh ở `scripts/verify-g0.mjs#PILOT_INLINE_PERM_ROUTES` | events+attachments | — | có (xoá file vật lý) | routes.js:1365 |
| R092 | POST | /api/events/:id/costs | **`requirePerm(events,create)` tương đương, kiểm INLINE trong handler theo action `create` cho đúng thực tế (trước batch này map nhầm sang `edit` — D13 batch `RBAC-EXP-B4`, entity Inherited)**: router-level `requireAuth` + `policyService.prepareCreate()`/PolicyEngine, module `events`/action `create`, `parentOwnerId` = `owner_id` của event cha (event_cost không có owner_id riêng) — khai tường minh ở `scripts/verify-g0.mjs#PILOT_INLINE_PERM_ROUTES` | event_costs | amount (không còn "KHÔNG gate write" — F1 đã đóng cho entity này) | không | routes.js:1380 |
| R093 | PUT | /api/events/:id/costs/:cid | **`requirePerm(events,edit)` tương đương, kiểm INLINE trong handler (D13 batch `RBAC-EXP-B4`, entity Inherited)**: router-level `requireAuth` + `policyService.assertWritable()`/PolicyEngine, module `events`/action `edit`, `parentOwnerId` = `owner_id` của event cha — executor chỉ qua khi parentOwnerId = chính họ — khai tường minh ở `scripts/verify-g0.mjs#PILOT_INLINE_PERM_ROUTES` | event_costs | amount (F1 đã đóng) | không | routes.js:1392 |
| R094 | DELETE | /api/events/:id/costs/:cid | **`requirePerm(events,delete)` tương đương, kiểm INLINE trong handler (D13 batch `RBAC-EXP-B4` — SỬA ĐÚNG: trước đây map nhầm vào quyền `edit` nên executor xoá được, nay chỉ Admin/Super Admin)**: router-level `requireAuth` + `policyService.assertWritable()`/PolicyEngine, module `events`/action `delete` — khai tường minh ở `scripts/verify-g0.mjs#PILOT_INLINE_PERM_ROUTES` | event_costs | — | không | routes.js:1402 |
