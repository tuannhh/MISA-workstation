...
### F13 — Bộ nhắc việc/thông báo KHÔNG chạy được trên MySQL (production) · **High / B / browser-production — ĐÃ FIX** (G1A.3 commit 4, phát hiện qua characterization; sửa ngay trong G1A.3 commit 5 theo yêu cầu owner)
...
- **Production Cloud Run hiện chạy `DB_CLIENT=mysql`** (xem `13-deployment-runbook.md`) — nếu đã từng có mốc nhắc + user `notify_opt_in=1` đến hạn từ khi deploy, tính năng này chưa từng tạo ra thông báo nào trên production trước khi có bản fix này.
