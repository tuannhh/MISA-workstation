...
### F13 — Bộ nhắc việc/thông báo KHÔNG chạy được trên MySQL (production) · **High / B / browser-production — ĐANG SỐNG** (G1A.3 commit 4, phát hiện qua characterization)
`server/scheduler.js:28` dùng `WHERE ... AND (recipient_user_id IS ? OR recipient_user_id=?)` — cú pháp `IS ?` chỉ hợp lệ với driver SQLite (`node:sqlite` chấp nhận `IS` như so sánh tổng quát có xử lý NULL); MySQL chỉ cho `IS` đi với từ khoá `NULL/TRUE/FALSE/UNKNOWN`, không cho placeholder tham số → mọi lần gọi thực sự chạm nhánh này ném `ER_PARSE_ERROR` ("You have an error in your SQL syntax ... near '1 OR recipient_user_id=1)'").
...
- **Production Cloud Run hiện chạy `DB_CLIENT=mysql`** (xem `13-deployment-runbook.md`) — nếu đã từng có mốc nhắc + user `notify_opt_in=1` đến hạn từ khi deploy, tính năng này chưa từng tạo ra thông báo nào trên production.
- Phát hiện qua characterization G1A.3 (`server/test/integration-reminders.test.js`, test `R060`/`R057-059` viết driver-aware `isMysql ? 400 : 200`) — **không sửa `scheduler.js` ở commit này**, đúng nguyên tắc không đổi nghiệp vụ khi characterization. Fix thật: bỏ hẳn `IS ?`, viết lại điều kiện NULL-safe theo kiểu tách 2 câu (`recipient_user_id IS NULL` khi tham số null, else `recipient_user_id=?`) hoặc dùng `<=>` (NULL-safe equal, MySQL hỗ trợ, SQLite không) sau khi kiểm tra tương đương ở cả 2 driver.
- Đưa vào G1B (remediation có test trước/sau) hoặc W1 tuỳ owner ưu tiên — **đề xuất ưu tiên cao vì đang ảnh hưởng production thật**, khác các finding F1-F12 vốn là rủi ro/lỗ hổng chưa chắc đã bị khai thác.
