...
Response `GET /partners/:id` (detail): `record` (đã mask) + **7 mảng con** trong 1 response — `people`, `sponsorships`, `interactions`, `dates` (important_dates), `fees` (association_fees, `amount` bị thay `'●●● (đã ẩn)'` thủ công nếu thiếu `org_fee` — cách mask khác với `rbac.maskRecord`, xem `routes.js:145`), `agreements`/`workLogs` (mỗi item kèm `files: [...]` là danh sách attachment `kind='file'` gắn theo owner), `gifts`, `benefitUsages`, `caretakers`. Đây là 1 API "gộp" toàn bộ dữ liệu liên quan 1 cơ quan để tránh nhiều round-trip — client (`public/app.js`) render tất cả từ 1 lần gọi.
