...
1. **Vue 3 shell** (`frontend/src/`) — build bằng Vite (`vite.config.mjs`), output thẳng vào `public/assets/vue-app.js` + `vue-app.css` (`emptyOutDir: false` — không xoá `public/` khi build, `vite.config.mjs:14`). Chỉ chịu trách nhiệm: khung trang đăng nhập (`#login`), khung app sau đăng nhập (header, sidebar, settings modal đổi theme/density — `App.vue:80-149`), và các phần tử DOM rỗng làm "hố cắm" cho legacy script (`#nav`, `#view`, `#crumb`, `#bellPanel`, `#modalRoot`, `#toast` — `App.vue:86,91,100,105,109-110`).
...
**Kết luận không suy đoán**: MDS hiện chỉ áp dụng cho ~5% diện tích UI thực tế (khung ngoài), còn ~95% nội dung nghiệp vụ (mọi bảng/form/detail của 12 module) render bằng HTML-string tự viết, không đi qua bất kỳ component MDS nào. Đây chính là lý do cụ thể (không phải nhận định chung) khiến quyết định **D4 (strangler theo vertical slice)** ở [`02-decisions.md`](02-decisions.md) là cách duy nhất khả thi để đưa MDS thật vào — big-bang rewrite 2.889 dòng HTML-string + toàn bộ 105 lời gọi `api()` đi kèm là rủi ro không chấp nhận được với dữ liệu production đang sống. Slice đầu tiên đã chốt là **People Detail** (**D5**) — đúng entity có bề mặt dữ liệu mật lớn nhất (contact/private/social/finance/iddoc/org_fee), vì đây vừa là nơi rủi ro bảo mật cao nhất (F1) vừa là nơi cần kiểm chứng pattern "characterization test → extract → Desktop MDS → Native composition" trước khi nhân rộng ra 11 module còn lại.

## E. Native mobile — chưa tồn tại (F5, tham chiếu)

`app.js` chỉ có 1 media query ẩn sidebar ở màn hình hẹp (responsive web, không phải native) — không có `.mds-mobile-app`, bottom nav, safe-area, hay bất kỳ native composition nào. Chi tiết đầy đủ đã có ở F5/F6 trong [`01-audit-findings.md`](01-audit-findings.md) và Section C của [`08-permission-matrix.md`](08-permission-matrix.md) — không lặp lại ở đây.
