...
**Hiện trạng 2026-08-25:** G1A.1 (harness) đã qua 2 vòng remediation sau audit (round 1 → HOLD/PARTIAL 5 blocker; round 2 → Codex re-audit "gần CLOSE" còn 3 blocker R2-01/R2-02/R2-03) — **remediation round 3 đã nộp, chờ Codex re-audit lần 3 trước khi ghi XONG** (xem dòng G1A.1 dưới); G1A.9 vẫn chỉ ở mức khung (147/147 dòng `TODO`). Còn G1A.2-G1A.8 + G1B/G1C. **Ước lượng:** 2,5-4 tuần (điều chỉnh tăng so với bản trước — Codex round-3 re-audit R3-07/C0.6 bác bỏ cách thu hẹp "chỉ phần giữ nguyên", yêu cầu characterization-core phủ TOÀN BỘ 145 route + mọi job/business-flow, không chỉ nhóm module không đổi).
...
|---|---|---|
| G1A.1 | **PARTIAL — remediation round 3 nộp 2026-08-25, CHỜ Codex re-audit lần 3, KHÔNG tự ghi XONG.** Round 1 bị Codex trả `HOLD/PARTIAL` (5 blocker A1-A5, xem changelog). Round 2 remediate A1-A5 nhưng Codex re-audit round 2 vẫn `HOLD/PARTIAL` — "đã gần CLOSE" nhưng còn 3 blocker: (R2-01) `after()` có thể tự `require('../db')` và khởi tạo/seed DB mặc định nếu `before()` thất bại trước khi DB module được load; (R2-02) `dropMysqlTestDb()` chưa cần opt-in, rollback/worker-close vẫn nuốt lỗi, allowlist host chưa phân biệt Docker test với Cloud SQL Proxy local cùng loopback; (R2-03) verifier vẫn hard-code cặp `[file, prefix]` cho router mount `/api`/`/api/ai` thay vì parse thật từ `server/app.js`. **Đã remediate trong round 3:** smoke.test.js đổi sang pattern resource-stack (`server/test-support/resource-stack.js`) — `after()` chỉ cleanup đúng resource đã acquire thành công, không bao giờ `require()` mới; thêm 3 failure test (`server/test/smoke-failure.test.js`) tái hiện đúng 3 kịch bản Codex nêu (create/grant failure trước require, listen failure sau khi DB đã load, init failure sau worker spawn — kịch bản cuối chạy trong process con riêng vì db.js có side-effect module-scope, chứng minh process crash rõ ràng exit≠0 chứ không treo, residual risk ghi rõ trong changelog); `dropMysqlTestDb()` giờ cũng gọi `assertOptIn()`; rollback/worker-close error không còn bị nuốt (`error.cleanupError` ở `db-harness.js`, `shutdownAck`+reject ở `mysql-worker.js`/`mysql-sync.js`); tách namespace biến bootstrap admin thành `TEST_MYSQL_HOST/PORT/ADMIN_USER/ADMIN_PASSWORD` riêng (không kế thừa `MYSQL_*` của app); chặn `MYSQL_SOCKET_PATH` (Cloud SQL production); validate `MYSQL_USER` bằng regex trước khi dùng trong GRANT; `resolveRouterPrefix()`/`parseRouterMounts()` parse thật `app.use(prefix, routerVar)` từ `server/app.js`, có negative self-test chứng minh phản ánh đúng khi mount đổi/bị xoá; test HTTP server bind `127.0.0.1` thay vì mọi interface (S1); sửa restore env dùng delete thay vì gán chuỗi `'undefined'` (S2 — **phát hiện đúng bug này khi tự chạy lại suite round 3**, một test khác bị fail thật vì lỗi restore env sai, đã sửa và xác nhận lại xanh). Evidence đầy đủ: xem `15-changelog.md` mục 2026-08-25 remediation round 3 | test / all / `test:integration:sqlite` 6 pass+5 skip, `test:integration:mysql` 11/11 pass exit 0 (~1,9s), `test:verify-g0-selftest` 6/6 pass, `verify-g0.mjs` PASS, `git diff --check` sạch, 0 database `pr_media_test_%` sót lại, 0 process còn sống — CHƯA phải Codex CLOSE |
