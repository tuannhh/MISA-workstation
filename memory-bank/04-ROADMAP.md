...
|---|---|---|---|
| P0 cho hoàn tất MDS | **W2.2 / F6:** tiếp tục thay legacy UI theo vertical slice. `public/app.js` vẫn là fallback 2.908 dòng/HTML-string cho các route chưa có Vue feature; `App.vue` đã mount Dashboard, People, Partner list/detail, Supplier list/detail, Interactions, Events, Awards, Monitoring, Reports, Admin, Reminders. | Dashboard, 4 loại Partner list/create và Supplier list/create đã có domain/API + Desktop MDS + Native composition; **Reports Detail (nhân sự/đơn vị/giải thưởng/cảnh báo) đã tách 2026-09-02**. Còn money subflow, monitor detail và reminder AI/ICS. | Mỗi slice: characterization → domain dùng chung → Desktop MDS + Native composition riêng → role/403/local-native test → evidence bundle. Không big-bang rewrite hoặc xóa fallback sớm. |
| P0 cho tác nghiệp | Hoàn tất các flow legacy còn lại theo nhóm: (1) money subflows: budget, partner sponsorship/fee/gift/benefit, supplier quote/transaction/contact mutation; (2) report chi tiết; (3) monitor scan/runs/highlights/settings/alerts/competitor/campaign; (4) reminder ICS + AI card text/image. | Các endpoint/flow này vẫn chỉ có legacy `VIEWS` hoặc chưa có native composition. | Không gọi là “100% native” trước khi từng flow có route/surface/role evidence; ưu tiên theo giá trị nghiệp vụ và dữ liệu nhạy cảm, không theo số dòng. |
