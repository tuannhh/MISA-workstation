...
|---|---|---|---|
| P0 cho hoàn tất MDS | **W2.2 / F6:** tiếp tục thay legacy UI theo vertical slice. `public/app.js` vẫn là fallback 2.908 dòng/HTML-string cho các route chưa có Vue feature; `App.vue` đã mount Dashboard, People, Partner list/detail, Supplier list/detail, Interactions, Events, Awards, Monitoring, Reports, Admin, Reminders. | Dashboard, 4 loại Partner list/create và Supplier list/create đã có domain/API + Desktop MDS + Native composition; còn money subflow, report detail, monitor detail và reminder AI/ICS. | Mỗi slice: characterization → domain dùng chung → Desktop MDS + Native composition riêng → role/403/local-native test → evidence bundle. Không big-bang rewrite hoặc xóa fallback sớm. |
