...
| G1A.3 | Integration API cho **ĐỦ 145 route** (không giới hạn nhóm module) — request/response shape, validate, not-found/conflict, side-effect DB, tính toán, audit, file/download, side-effect job nền; case tối thiểu mỗi route: happy + invalid + unauthenticated + not-found. **ĐANG LÀM — chuyển sang fast-track protocol (`17-fast-track-collaboration.md`, owner approved `ffdc3df`, 2026-08-26): audit theo batch 15-25 route, không theo từng commit.** Batch "reports-awards" (R046-R056 + R062-R071, 21 route, 3 commit `f40be9c`/`119f81a`/`c0270a1`) đã xong, **chờ gửi Codex ONE-SHOT AUDIT** — xem Evidence Bundle trong `15-changelog.md`. Phát hiện+fix ngay **F14** (grandTotal nối chuỗi trên MySQL, owner duyệt fix cùng cơ chế F13) trong lúc code batch; phát hiện **F15** (FK award_participations không thực thi trên MySQL) ghi backlog P2, không sửa (đúng scope-freeze). Mapping sau batch: 79/145 green, 66 TODO. Kế hoạch chia batch tiếp theo: suppliers(R072-R086) → events+dashboard(R087-R098) → monitor phần 1(R104-R120) → monitor phần 2(R121-R135) → ai(R136-R142) | test / — / — |
