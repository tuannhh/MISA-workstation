...
**Hiện trạng 2026-08-25:** G1A.1 (harness) đã qua 3 vòng remediation sau audit (round 1 → HOLD/PARTIAL 5 blocker A1-A5; round 2 → còn R2-01/R2-02/R2-03; round 3 → Codex re-audit vẫn `HOLD/PARTIAL` với 2 blocker sâu hơn R3-01/R3-02 + 1 mục tự hạ xuống backlog R3-03) — **remediation round 4 đã nộp, chờ Codex re-audit lần 4 trước khi ghi XONG** (xem dòng G1A.1 dưới); G1A.9 vẫn chỉ ở mức khung (147/147 dòng `TODO`). Còn G1A.2-G1A.8 + G1B/G1C. **Ước lượng:** 2,5-4 tuần (điều chỉnh tăng so với bản trước — Codex round-3 re-audit R3-07/C0.6 bác bỏ cách thu hẹp "chỉ phần giữ nguyên", yêu cầu characterization-core phủ TOÀN BỘ 145 route + mọi job/business-flow, không chỉ nhóm module không đổi).
...
|---|---|---|
| G1A.1 | **PARTIAL — remediation round 4 nộp 2026-08-25, CHỜ Codex re-audit lần 4, KHÔNG tự ghi XONG.** Round 1: 5 blocker A1-A5. Round 2: remediate A1-A5 nhưng re-audit round 2 còn R2-01/R2-02/R2-03. Round 3: remediate R2-01/R2-02/R2-03 (resource-stack, opt-in cho cả DROP, router-mount verifier parse thật, loopback bind, restore-env) nhưng Codex re-audit round 3 phát hiện thêm 2 blocker sâu hơn trong đúng phần vừa sửa: (R3-01) `init()`/`seed()` throw trong `before()` **thật** của `node:test` (không phải script thường) vẫn làm process treo — Codex đo được phải `SIGKILL` sau 6s; (R3-02) `MySQLSyncDatabase.close()` có bug TDZ (`clearTimeout(timer)` trước khi `timer` được khai báo, che mất lỗi gốc khi `postMessage()` throw đồng bộ) và false-success (`worker.once('exit', ...)` bỏ qua exit code, nên worker exit 1 mà mất/chưa kịp gửi `shutdownAck` vẫn được coi là đóng thành công). Codex đồng thời **tự hạ** (R3-03) từ blocker xuống SHOULD-FIX/backlog DevOps (xem `G1A.10`): namespace `TEST_MYSQL_*` chưa chứng minh phân biệt được Docker test với Cloud SQL Proxy local cùng loopback — xác suất thấp trong môi trường local kiểm soát, không đáng giữ gate. **Đã remediate trong round 4:** `server/db.js` bọc `try/catch` quanh `dropAll()/init()/seed()` — nếu throw, đóng worker (`db.close()`, không chờ đồng bộ) TRƯỚC khi rethrow, nên process luôn tự thoát nhanh dù lỗi xảy ra trong `before()` thật (fixture mới `server/test-support/db-init-hang-fixture.js` + test dùng đúng `node --test` lồng nhau tái hiện chính xác kịch bản Codex, đo elapsed <4s, không cần SIGKILL — **trong lúc viết test này tự phát hiện thêm 1 bug: `node --test` lồng nhau kế thừa `NODE_TEST_CONTEXT`/`NODE_TEST_WORKER_ID` từ process cha khiến con tưởng đang chạy đệ quy và tự skip toàn bộ, phải xoá 2 biến này trước khi spawn con**); `mysql-sync.js` tách `closeWorker()` thành hàm thuần nhận worker-like object, khai báo `timer` trước khi gọi `postMessage()` (hết TDZ), chỉ resolve khi có đúng 1 `shutdownAck` không lỗi VÀ exit code 0 — mọi tổ hợp khác (ack lỗi/exit khác 0/exit trước ack/`error` event/`postMessage` throw đồng bộ/force-terminate) đều reject; thêm 8 unit test bằng fake worker (`server/test/mysql-sync-close.test.js`) phủ từng nhánh transition. R3-03 chuyển ghi nhận backlog ở `G1A.10`, không implement round này theo đúng đề xuất tối ưu chi phí/rủi ro của Codex. Evidence đầy đủ: xem `15-changelog.md` mục 2026-08-25 remediation round 4 | test / all / `test:integration:sqlite` 14 pass+6 skip, `test:integration:mysql` 20/20 pass exit 0 (~1,8-2s), `test:verify-g0-selftest` 6/6, `verify-g0.mjs` PASS, `git diff --check` sạch, 0 database `pr_media_test_%` sót lại, 0 process còn sống — CHƯA phải Codex CLOSE |
