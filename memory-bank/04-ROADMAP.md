...
| G1A.3 | Integration API cho **ĐỦ 145 route** (không giới hạn nhóm module) — request/response shape, validate, not-found/conflict, side-effect DB, tính toán, audit, file/download, side-effect job nền; case tối thiểu mỗi route: happy + invalid + unauthenticated + not-found. **ĐANG LÀM — Codex audit F13 xong 2026-08-25 (ACCEPTED, mapping 55/145 green, 90 TODO, vẫn OPEN).** Commit 4 = reminders/notifications (R038-R042,R057-R061, 24 test) → commit 5 = fix **F13** (`scheduler.js` bỏ cú pháp `IS ?` chỉ SQLite chấp nhận) → commit 6 = 2 việc Codex nêu không chặn tiến độ: sửa drift thuật ngữ "production"→"môi trường test" ở `01-audit-findings.md`, và cách ly `DATA_DIR` cho `createMysqlTestDb()` (trước đây leak vào `data/uploads` thật khi quên set tay — đã xác nhận 88 file rác thật trong session, xem `15-changelog.md`) — **điều kiện Codex nêu trước khi đóng G1A.3 đã xử lý xong**, không phải chờ thêm. Kế hoạch chia commit theo module (mirror nhóm view ở `07-route-catalog.md`): auth+admin ✅ → partners(R001-R028) ✅ → people(R029-R037) ✅ → reminders/notifications(R038-R042,R057-R061) ✅ (+ fix F13 + DATA_DIR isolation) → interactions(R043-R045) → reports/bookings/budgets(R046-R056) → awards(R062-R071) → suppliers(R072-R086) → events+dashboard(R087-R098) → monitor phần 1(R104-R120) → monitor phần 2(R121-R135) → ai(R136-R142). Gộp báo cáo Codex sau vài commit, không phải từng commit một | test / — / — |
