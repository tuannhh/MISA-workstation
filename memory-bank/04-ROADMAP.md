...
| **O4 (session store) + O5 (SLO/concurrency) → DevOps MISA** | W1.7 vẫn dựng **seam** durable-session (interface + fail-closed) nhưng **backend cụ thể (bảng SQL/Redis) do DevOps chọn** khi cấu hình production thật. W2.3 (acceptance SLO gate) **con số SLO + topology do DevOps chốt** — Claude dựng harness đo, không tự đặt ngưỡng release. |
...
| **D14 — Voice Assistant vision** (gọi từ mọi màn hình mobile + AI tự tìm entity + tự ghi nhận) | Tính năng MỚI lớn, **mở rộng nhiều** so với `POST /ai/interaction-voice` hiện có (chỉ trích-xuất-chờ-duyệt). Thành 1 track riêng ở Wave 3/4, chặn bởi O3 (native host) + O8 (Security/Legal) + **3 câu hỏi thiết kế D14.2 chưa trả lời** (bỏ human-in-the-loop hay giữ? xử lý match sai entity? quy tắc AI đổi relationship_score?). |
...
Đường tới hạn thật: **O3** (contract auth AMIS) và **O8** (data policy + AI tự-ghi) — owner phải gửi **ngay** (lead-time dài), chặn W1.AI-POLICY/W3-voice/W4, KHÔNG chặn G1/W1.RBAC/W1.8/W1.9/W2.5.
...
**Trạng thái 2026-08-24: phần Claude làm được ĐÃ XONG; chờ Codex re-audit round 3 + owner gửi O3/O8.**

| # | Task | Trạng thái |
|---|---|---|
| G0.1 | Owner quyết O1-O8 + gửi O3/O8 | **PHẦN LỚN XONG (2026-08-24):** O6 APPROVED ($200); O2/O7 SUPERSEDED bởi D13; O4/O5 DEFERRED-TO-DEVOPS; O1 mặc định `private` (chờ owner không phản đối); D13/D14 đã ghi. **CÒN LẠI: O3 (gửi team AMIS) + O8 (gửi Security/Legal) chưa SUBMITTED** — hành động của owner, exit G0.1 chờ 2 cái này đạt tối thiểu `SUBMITTED`. |
...
**Exit gate G0 (cập nhật):** tất cả trên xong TRỪ O3/O8 submission (owner action). Codex re-audit round 3 xác nhận G0.4/G0.5/G0.8 đã sửa đúng + G0.2 đủ coverage → khi đó G0 đóng được (O3/O8 submission là điều kiện song song, owner tự làm).
...
| W1.AI-POLICY | Data-egress enforcement: deny-by-default, tier/purpose allowlist, redact/minimize, consent voice, retention/delete, egress audit KHÔNG chứa payload, kill-switch `AI_DISABLED`, negative tests. **Bao cả 12 luồng egress + SMTP (`06-threat-model.md` §D)** | F4 | **CẦN O8** | code+test / — / fail-closed deny-by-default |
...
| # | Task | Phụ thuộc | Evidence Contract |
|---|---|---|---|
| W3.VOICE.0 | **Owner/BA trả lời 3 câu D14.2 TRƯỚC khi thiết kế:** (a) giữ bước xác nhận human-in-the-loop hay AI tự ghi thẳng? (b) xử lý match sai/nhập nhằng entity thế nào? (c) quy tắc AI đổi `relationship_score`? | owner/BA | doc / — / block thiết kế nếu chưa trả lời |
| W3.VOICE.1 | Redesign route AI voice: từ "trích-xuất-chờ-duyệt" (hiện có, an toàn) → có thể "tự hành động" với rào chắn: confidence threshold match entity, giới hạn hành động AI tự làm, **log đầy đủ mọi lần AI tự ghi để audit/rollback** | W3.VOICE.0, W1.AI-POLICY, O8 | code+test / — / fail-closed khi confidence thấp |
| W3.VOICE.2 | "Gọi từ mọi màn hình" — cần trigger toàn cục native (floating mic/assistant) — **gắn O3 (native host), không làm thuần web** | O3, W2.5 | code+device-test / native / `UNVERIFIED` tới O3 |

---

## WAVE 4 — Native runtime & release gate (+ voice runtime)
...
| W4.2 | QA ma trận MDS: 375/393/412/768/1024, portrait/landscape/split, iOS Dynamic Type/Zoom, Android Font/Display, keyboard/Back/gesture | device-test / native / — |
| W4.3 | Permission denied/revoked/unavailable; restore draft; deep-link 403; gesture fallback; accessibility OS | device-test / native / fail-closed |
| W4.VOICE | Voice Assistant runtime trên native: mic OS permission, gọi từ mọi màn hình, AI tự-ghi có audit/rollback thật trên thiết bị | device-test / native / cần O8 + W3.VOICE |
...
**Exit gate W4 (= release gate):** ma trận scope 4-vai-trò không còn `UNVERIFIED` cho route release; runtime evidence trên host/thiết bị thật; security gate rỗng; W2.3 PASS; scorecard ≥8,5, không trục critical <8,0. **Release có AI/voice bật thêm yêu cầu O8 duyệt + W3.VOICE.0 trả lời.** Build `AI_DISABLED` release native shell pilot được, gọi đúng tên "native shell, AI tắt".
...
| F4/O8 AI governance | G0.8 + O8 → W1.AI-POLICY → W3.VOICE (D14) → W4.VOICE | mở rộng theo D14 |
| F5 mobile native | G0.4 (matrix) → W2.5 (contract-ready) → slice Wave 3 → W4.1-4.3 (runtime) | |
...
| D14 Voice | W3.VOICE.0-2 + W4.VOICE | mới, cần O3+O8+D14.2 |

## Điều kiện release đầy đủ MDS/native (không đổi tinh thần)
Không tuyên bố "100% MDS/native" khi: matrix 4-vai-trò còn `UNVERIFIED` cho route release, HOẶC provider AMIS thật (O3) chưa qua contract+device test (W4.1), HOẶC W2.3 chưa PASS, HOẶC (nếu bật AI/voice) O8 chưa duyệt / D14.2 chưa trả lời. Build `AI_DISABLED` release native shell pilot riêng — gọi đúng tên "native shell, AI tắt".
