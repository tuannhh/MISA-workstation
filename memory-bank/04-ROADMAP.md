...
| G1A.4 | **XONG (Bundle A phần 1) — commit `<pending>`, chưa audit.** Reminder check-then-insert (`11-business-flows.md` §E, R3-02C): test mới `server/test/integration-jobs.test.js` gọi thẳng `scheduler.runOnce()` — happy (log in-app đúng ngày/seq), idempotent khi gọi tuần tự, và **CHARACTERIZATION xác nhận tiền đề DB-level của race hiện tại** (chèn trực tiếp 2 dòng `reminder_log` giống hệt nhau, không có UNIQUE constraint chặn — không phải test hành vi đích, không mô phỏng race đồng thời thật trong 1 process vì `runOnce()` không có điểm yield giữa check và insert) | test / — / characterization, không phải target |
...
| G1A.7 | **ĐANG LÀM (Bundle A phần 2) — commit `<pending>`, CHƯA đóng, đã bàn giao Codex tiếp tục (2026-08-26, hết token phiên Claude).** `server/test/integration-ai-golden.test.js` (22 test): golden happy CÓ API key + fetch mock (fake/fixture, KHÔNG gọi Gemini/Internet thật — `process.env.GEMINI_API_KEY` set trước mọi require, `global.fetch` thay bằng hàng đợi response) cho ĐỦ 12 luồng threat-model AI-E001..AI-E012 (`06-threat-model.md` §A) + 4 test CHARACTERIZATION lỗi dùng chung tầng `gemini.js call()` (malformed JSON/quota HTTP 429/timeout AbortError/genImage không trả ảnh — quyết định test 1 lần ở tầng `call()` thay vì lặp 12 lần, cùng tiền lệ "cùng root cause test 1 lần" F13-F18, vì mọi luồng genJSON/genText/genImage/groundedSearch đều đi qua đúng 1 hàm `call()` xử lý lỗi HTTP giống hệt nhau). **SQLite: 18/18 GREEN.** **MySQL: 15/18, 3 FAIL THẬT (không phải flaky bị bỏ qua)** — phát hiện 2 vấn đề mới: **F19** (P1, `POST /ai/interaction-voice` 502 mọi lần trên MySQL vì `COLLATE NOCASE` không hợp lệ — root cause đã xác định rõ, hướng sửa đã đề xuất, CHƯA SỬA) và **F20** (nghi vấn, CHƯA XÁC ĐỊNH ROOT CAUSE — `groundIngest()`/`siteGroundIngest()` báo ghi thành công nhưng SELECT lại không thấy dòng trên MySQL, nghi cơ chế đồng bộ `mysql-worker.js`, CÓ THỂ là bug rộng hơn ảnh hưởng mọi ghi-rồi-đọc-ngay trên MySQL — cần điều tra kỹ trước khi kết luận mức độ). Chi tiết đầy đủ: `01-audit-findings.md` §F19/§F20. **KHÔNG được coi G1A.7 CLOSE cho tới khi MySQL đạt 18/18 GREEN** — đây là gate quan trọng vì chính G1A.5 (DB contract dual-driver) cũng phụ thuộc kết luận F20 có đúng là lỗi hạ tầng MySQL rộng hay không | test / all / **PARTIAL — SQLite green, MySQL red, bàn giao Codex tiếp** |
| G1A.8 | **XONG (Bundle A phần 1) — commit `<pending>`, chưa audit.** `server/test/integration-jobs.test.js` verify side-effect DB thật (không chỉ route HTTP): `JOB-REMINDER` (đã gộp ở G1A.4) + `JOB-MONITOR-SCAN` — gọi thẳng `monitor.runScan({triggeredBy:'auto'})` KHÔNG truyền `queryIds` (đúng đường job tự động thật, khác route thủ công `POST /monitor/scan` luôn yêu cầu `query_ids` tường minh), network-safe (tắt hết `sources`, rỗng `include`), verify `scan_runs.queries` = đúng số query đang enabled; `monitor.applySchedule()` verify không leak timer qua theo dõi `setInterval`/`clearInterval` | test / — / — |
