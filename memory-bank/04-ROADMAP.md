...
| G1A.3 | Integration API cho **ĐỦ 145 route** (không giới hạn nhóm module) — request/response shape, validate, not-found/conflict, side-effect DB, tính toán, audit, file/download, side-effect job nền; case tối thiểu mỗi route: happy + invalid + unauthenticated + not-found. **ĐANG LÀM — commit 5/~12 xong 2026-08-25** (fix **F13** ngay theo yêu cầu owner — `scheduler.js` bỏ cú pháp `IS ?` chỉ SQLite chấp nhận, đã pass cả 2 driver; commit 4 là reminders/notifications, R038-R042,R057-R061, 24 test, `server/test/integration-reminders.test.js` — xem `15-changelog.md` + `01-audit-findings.md` F13). Kế hoạch chia commit theo module (mirror nhóm view ở `07-route-catalog.md`): auth+admin ✅ → partners(R001-R028) ✅ → people(R029-R037) ✅ → reminders/notifications(R038-R042,R057-R061) ✅ (+ fix F13) → interactions(R043-R045) → reports/bookings/budgets(R046-R056) → awards(R062-R071) → suppliers(R072-R086) → events+dashboard(R087-R098) → monitor phần 1(R104-R120) → monitor phần 2(R121-R135) → ai(R136-R142). Gộp báo cáo Codex sau vài commit, không phải từng commit một | test / — / — |
