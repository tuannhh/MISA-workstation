...
|---|---|---|---|
| P0 cho hoàn tất MDS | **W2.2 / F6:** tiếp tục thay legacy UI theo vertical slice. `public/app.js` vẫn là fallback 2.908 dòng/HTML-string cho các route chưa có Vue feature; `App.vue` mới mount các slice People, Partner **detail**, Supplier **detail**, Interactions, Events, Awards, Monitoring, Reports, Admin, Reminders. | `public/app.js` vẫn giữ `dashboard`, 4 danh sách partner, supplier list/create, budgets, report tabs và các tab monitor; `frontend/src/features/` chưa có Dashboard, PartnerList, SupplierList, Budget hay report-detail feature. | Mỗi slice: characterization → domain dùng chung → Desktop MDS + Native composition riêng → role/403/local-native test → evidence bundle. Không big-bang rewrite hoặc xóa fallback sớm. |
| P0 cho tác nghiệp | Hoàn tất các flow legacy còn lại theo nhóm: (1) dashboard/press overview + Partner/Supplier list/create; (2) money subflows: budget, partner sponsorship/fee/gift/benefit, supplier quote/transaction/contact mutation; (3) report chi tiết; (4) monitor scan/runs/highlights/settings/alerts/competitor/campaign; (5) reminder ICS + AI card text/image. | Các domain Vue hiện chỉ gọi `GET /reports`, reminders CRUD/inbox và monitoring dashboard/source/mention/query; Admin field visibility cũng đang khóa `module:'partners'`. Các endpoint/flow còn lại vẫn chỉ có legacy `VIEWS` hoặc chưa có native composition. | Không gọi là “100% native” trước khi từng flow có route/surface/role evidence; ưu tiên theo giá trị nghiệp vụ và dữ liệu nhạy cảm, không theo số dòng. |
