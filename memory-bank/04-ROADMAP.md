...
|---|---|---|---|
| P0 cho hoàn tất MDS | **W2.2 / F6:** tiếp tục thay legacy UI theo vertical slice. `public/app.js` vẫn là fallback 2.908 dòng/HTML-string cho các route chưa có Vue feature; `App.vue` đã mount Dashboard, People, Partner list/detail, Supplier **detail**, Interactions, Events, Awards, Monitoring, Reports, Admin, Reminders. | Dashboard và 4 loại Partner list/create đã có domain/API + Desktop MDS + Native composition + local-native evidence; còn supplier list/create, budgets, report tabs và các tab monitor. | Mỗi slice: characterization → domain dùng chung → Desktop MDS + Native composition riêng → role/403/local-native test → evidence bundle. Không big-bang rewrite hoặc xóa fallback sớm. |
| P0 cho tác nghiệp | Hoàn tất các flow legacy còn lại theo nhóm: (1) Supplier list/create; (2) money subflows: budget, partner sponsorship/fee/gift/benefit, supplier quote/transaction/contact mutation; (3) report chi tiết; (4) monitor scan/runs/highlights/settings/alerts/competitor/campaign; (5) reminder ICS + AI card text/image. | Các domain Vue hiện chỉ gọi `GET /reports`, reminders CRUD/inbox và monitoring dashboard/source/mention/query; Admin field visibility cũng đang khóa `module:'partners'`. Các endpoint/flow còn lại vẫn chỉ có legacy `VIEWS` hoặc chưa có native composition. | Không gọi là “100% native” trước khi từng flow có route/surface/role evidence; ưu tiên theo giá trị nghiệp vụ và dữ liệu nhạy cảm, không theo số dòng. |
| P1 | Hoàn chỉnh điều hướng native giữa những slice đã có và danh sách/entry point của các slice mới. | `App.vue` native router đã nhận Dashboard, People, Person, Partner list/detail, Supplier detail, Interactions, Events, Awards, Monitoring, Reports, Admin, Reminders; chưa có supplier-list native resolver. | Không có navigation native nào rơi về legacy desktop; test route transition, Back, 403 và safe-area cho mỗi entry point. |
