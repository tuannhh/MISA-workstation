...
**Hiện trạng 2026-08-25:** G1A.1 (harness) remediation đã nộp sau Codex audit round 1 (HOLD/PARTIAL — xem dòng G1A.1 dưới), **chờ Codex re-audit tập trung theo 8 điều kiện close-contract trước khi ghi XONG**; G1A.9 vẫn chỉ ở mức khung (147/147 dòng `TODO`). Còn G1A.2-G1A.8 + G1B/G1C. **Ước lượng:** 2,5-4 tuần (điều chỉnh tăng so với bản trước — Codex round-3 re-audit R3-07/C0.6 bác bỏ cách thu hẹp "chỉ phần giữ nguyên", yêu cầu characterization-core phủ TOÀN BỘ 145 route + mọi job/business-flow, không chỉ nhóm module không đổi).
...
|---|---|---|
| G1A.1 | **PARTIAL — remediation nộp 2026-08-25, CHỜ Codex re-audit tập trung, KHÔNG tự ghi XONG.** Round 1 (harness ban đầu) bị Codex audit trả `HOLD/PARTIAL` với 5 blocker: (A1) `npm run test:integration:mysql` báo 2/2 xanh nhưng process treo vô hạn (worker MySQL/connection không đóng); (A2) setup lỗi ở bước GRANT để sót database tạm; (A3) harness dùng credential root + `MYSQL_HOST`/`MYSQL_PORT` kế thừa từ env, `dropMysqlTestDb` nhận tên tuỳ ý, Compose expose MySQL `0.0.0.0:3306` với password mặc định; (A4) roadmap ghi `XONG` dù tự thừa nhận thiếu fixture principal privileged; (A5) memory-bank + `scripts/verify-g0.mjs` bị lệch sau khi tách `app.js` (verifier hard-code 3 route auth nên vẫn báo PASS dù source đã đổi). **Đã remediate trong round này:** thêm `MySQLSyncDatabase.close()` + `closeDb()` (`server/mysql-sync.js`, `server/mysql-worker.js`, `server/db.js`) — `npm run test:integration:mysql` giờ exit code 0 thật, ~2s, không cần force-exit; `db-harness.js` rollback đúng khi GRANT lỗi (có failure-path test `server/test/db-harness-failure.test.js` tái hiện đúng thí nghiệm Codex, PASS) + GRANT theo đúng tên DB (không wildcard) + `dropMysqlTestDb` validate tên bằng regex + opt-in bắt buộc `ALLOW_TEST_DB_CREATE=1` + chặn `MYSQL_HOST` ngoài allowlist localhost; `docker-compose.yml` đổi `127.0.0.1:3306:3306` (đã verify `docker port` không còn `0.0.0.0`/`[::]`); thêm `fixtures.createPrivilegedUser()` (dùng `super_admin` sẵn có — đúng nghĩa "privileged" trong model 2-role hiện tại); sửa 8 file memory-bank + `scripts/verify-g0.mjs` (`sourceRoutes()` giờ parse 3 route auth thật từ `server/app.js`, không hard-code — có negative self-test `scripts/verify-g0.selftest.mjs` chứng minh parser fail khi route bị đổi/xoá, `npm run test:verify-g0-selftest` 3/3 PASS); `app-harness.close()` không còn nuốt lỗi + có timeout (N2); thêm rule DATA_DIR cho manual smoke vào `16-coding-rules.md` §B.8 (N1); thêm self-test cho clock helper (`server/test/clock-selftest.test.js`). Evidence đầy đủ: xem `15-changelog.md` mục 2026-08-25 remediation round 2 | test / all / `test:integration:sqlite` 4/5 pass+1 skip, `test:integration:mysql` 5/5 pass, `test:verify-g0-selftest` 3/3 pass — exit code 0 thật, 0 leak DB/process — CHƯA phải Codex CLOSE |
