...
| G1A.5 | **ĐÃ IMPLEMENT — chờ audit Bundle A.** `server/test/integration-db-contract.test.js` (4 contract tests): đủ 34 bảng; cột lõi + migration quan trọng; unique username + round-trip; app_meta upsert — chạy cả SQLite/MySQL. F15 (FK parity) vẫn là backlog Wave 1, không bị đóng băng thành contract xanh giả. | test / all / SQLite 4/4, MySQL 4/4; latest full suite SQLite 586 pass/7 skip, MySQL 592 pass/1 skip |
