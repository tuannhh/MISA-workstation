...
| G1A.8 | **ĐÃ IMPLEMENT — commit `99ddbec`, chờ audit Bundle A.** `server/test/integration-jobs.test.js` verify side-effect DB thật (không chỉ route HTTP): `JOB-REMINDER` (đã gộp ở G1A.4) + `JOB-MONITOR-SCAN` — gọi thẳng `monitor.runScan({triggeredBy:'auto'})` KHÔNG truyền `queryIds` (đúng đường job tự động thật, khác route thủ công `POST /monitor/scan` luôn yêu cầu `query_ids` tường minh), network-safe (tắt hết `sources`, rỗng `include`), verify `scan_runs.queries` = đúng số query đang enabled; `monitor.applySchedule()` verify không leak timer qua theo dõi `setInterval`/`clearInterval`. | test / — / — |
| G1A.9 | **MAPPING VERIFIER ĐÃ IMPLEMENT — `npm run test:verify-gate1-mapping` PASS.** Kiểm tra machine-readable: route catalog 145/145, mỗi route đúng một row, file/status hợp lệ; còn đúng 2 TODO target có owner/wave (`BR-AI-017`, `BR-AI-018`), không phải ô trống characterization. Gate 1 vẫn chờ G1B.1/.2/.3 và G1C/G1.8 theo exit gate. | test / all / `145/145`, 244 rows, TODO=2, known-red=0 |
...
| G1B.3 | **Test target-red viết xong 2026-08-28** (`server/test/target-session-f2.test.js`, batch contract `18-g1b-rbac-batch-contract.md#batch-g1b3-session-2026-08-28`), implement thật (regenerate session + rate-limit) vẫn thuộc W1.7, chưa làm. Dựng thêm cơ chế allowlist known-red bắt buộc theo G1B.6 (`server/test-support/known-red.js` + `memory-bank/g1b-allowlist.json`) vì trước đó chưa tồn tại. `F2-fixation` (session id không đổi khi login lại trên cookie có sẵn) và `F2-ratelimit` (không giới hạn số lần sai mật khẩu) đều RED đúng lý do, có allowlist `{owner:backend, expiry:2026-12-31}`. `F2-logout-invalidation` hoá ra đã GREEN sẵn (`session.destroy()` đã vô hiệu cookie cũ) — ghi nhận characterization, không đưa vào allowlist. Cả 2 driver xanh (SQLite 616, MySQL 616), chưa Codex audit. | test / — / RED allowlist `F2-fixation,F2-ratelimit` (2/2 có allowlist hợp lệ); `F2-logout` đã GREEN |
| G1B.4 | **XONG — F3 CLOSED 2026-08-27.** `safeFetch` chung chặn localhost/RFC1918/link-local/metadata/IPv6 private, DNS trả địa chỉ private, URL credential và redirect; transport pin IP đã verify để tránh DNS rebinding TOCTOU, cap body 2 MiB. Áp dụng monitor, grounding, create/update source và award-extract. Test BR-SSRF-010/011, BR-SSRF-021..028, R122/R139 xanh. `OUTBOUND_ALLOWED_HOSTS` là allowlist runtime tùy chọn, fail-closed khi được cấu hình. | code+test / all / GREEN |
| G1B.5 | **Test target-red viết xong 2026-08-28** (`server/test/target-n1-n2-explicit-permission.test.js`, batch `18-g1b-rbac-batch-contract.md#batch-g1b5-n1n2-2026-08-28`). N1/N2 đã RESOLVED (`02-decisions.md` §B.1) nhưng target chưa implement — 2 role hiện tại cấp full CRUD trên reminders/monitoring nên HTTP thật không phân biệt được `view` vs `ack`/`run` hôm nay; test đọc trực tiếp nguồn (route registration + rbac matrix), giống cách `verify-g0.mjs` khoá route catalog. `N1-explicit-action` (4 route side-effect còn dùng `view`) và `N2-dashboard-permission` (`/dashboard` chưa có `requirePerm`, `rbac.js` chưa có module `dashboard`) đều RED đúng lý do, allowlist `{owner:backend, expiry:2026-12-31, wave:W1}`. Implement thật vẫn thuộc Wave 1, chưa làm. | test / — / RED allowlist `N1-explicit-action,N2-dashboard-permission` (2/2 có allowlist hợp lệ) |
| G1B.6 | Mỗi RED ghi `{F-id/D13-target/N-id, owner, expiry}`; unexpected failure ngoài allowlist = build đỏ. **Hardened theo Codex audit Bundle A 2026-08-28:** bản đầu `known-red()` bắt MỌI exception (kể cả TypeError/lỗi fixture không liên quan) làm PASS giả — sửa bắt buộc tham số `expectedError` (RegExp/predicate), lỗi không khớp bị ném lại nguyên văn thay vì nuốt; có self-test qua child-process (`target-session-f2.test.js`) chứng minh cả 2 chiều. `verify-gate1-mapping.mjs` mở rộng đối chiếu allowlist: mỗi known-red row phải có entry `{id,owner,expiry,wave}` hợp lệ chưa hết hạn, không trùng, không mồ côi. | test / all / CI `security-gap`; verifier đối chiếu allowlist đã implement |
