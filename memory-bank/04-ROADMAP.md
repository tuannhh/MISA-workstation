...
>   G1.8 luôn đo MySQL ở mức 50 này. Stress liên tục 2026-09-02 có 2,53% lỗi/p95 12,73s; profile
>   W2.3 tác nghiệp 50 session, think time 5–15s sau đó đạt error rate 0 (p95 7,414s) tại
>   `993d300`. Đây là acceptance owner-bounded, **không phải SLO enterprise**. W2.4 async pilot
>   chỉ mở lại khi MISA không chấp nhận latency/budget, profile tương lai có lỗi, hoặc quy mô vượt 50.
...
| G1A.8 | **XONG — Codex ACCEPT Bundle A 2026-08-28 (commit `99ddbec`), follow-up hoàn tất 2026-09-02.** `server/test/integration-jobs.test.js` verify side-effect DB thật (không chỉ route HTTP): `JOB-REMINDER` (đã gộp ở G1A.4) + `JOB-MONITOR-SCAN` — gọi thẳng `monitor.runScan({triggeredBy:'auto'})` KHÔNG truyền `queryIds` (đúng đường job tự động thật, khác route thủ công `POST /monitor/scan` luôn yêu cầu `query_ids` tường minh), network-safe (tắt hết `sources`, rỗng `include`), verify `scan_runs.queries` = đúng số query đang enabled; `monitor.applySchedule()` verify không leak timer. Follow-up **BR-AI-018** thêm integration dual-driver cho `analyzePending()`: queue rỗng không egress, 10 rows chia 8+2, index map/chỉ ghi output hợp lệ, clamp score, và lỗi Gemini một chunk vẫn xử lý chunk sau. | test / SQLite+MySQL / GREEN |
| G1A.9 | **MAPPING VERIFIER ĐÃ IMPLEMENT — `npm run test:verify-gate1-mapping` PASS.** Kiểm tra machine-readable: route catalog 150/150, mỗi route đúng một row, file/status hợp lệ. `BR-AI-018` đã xanh ở G1A.8 follow-up; không còn test debt `TODO`. Known-red duy nhất là `INFRA-known-red-selftest`, fixture kiểm chính verifier/allowlist, **không phải finding sản phẩm**. | test / all / xem số liệu verifier tại HEAD |
...
| W2.3 | **XONG — F7 bounded-load owner acceptance (2026-09-02, `993d300`).** `npm run perf:think-time-50` dùng MySQL tạm, 50 session role-mix độc lập, initial stagger + think time đều 5–15s và status diagnostics theo workload. Artifact `perf-baseline/2026-09-02T08-50-21-609Z.json`: 229 request, **0 error**, p50 880ms/p95 7.414s/p99 8.964s, event-loop p99 686ms. Stress liên tục vẫn là FAIL/bottleneck và F7 không được xoá; profile này chỉ đạt ngưỡng owner “50 người tác nghiệp không lỗi”. | test / owner-bounded / PASS, không phải SLO enterprise |
...
> đều đã có evidence đóng.** Follow-up `BR-AI-018` đã được đóng bằng integration dual-driver; các việc dưới đây là phần còn lại
...
| Ưu tiên | Việc còn lại trong repo | Bằng chứng hiện trạng | Exit audit đúng nghĩa |
|---|---|---|---|
| P0 cho hoàn tất MDS | **W2.2 / F6:** tiếp tục thay legacy UI theo vertical slice. `public/app.js` vẫn là fallback 2.908 dòng/HTML-string cho các route chưa có Vue feature; `App.vue` đã mount Dashboard, People, Partner list/detail, Supplier list/detail, Interactions, Events, Awards, Monitoring, Reports, Admin, Reminders. | Dashboard, 4 loại Partner list/create/detail, Supplier list/create/detail/direct records, Reports Detail và Reminder ICS đã có domain/API + Desktop MDS + Native composition. **Monitoring operations (runs, competitors, campaigns, settings) vừa được tách thêm 2026-09-02**; AI highlights/brief card owner-deferred. | Mỗi slice: characterization → domain dùng chung → Desktop MDS + Native composition riêng → role/403/local-native test → evidence bundle. Không big-bang rewrite hoặc xóa fallback sớm. |
| P0 cho tác nghiệp | Hoàn tất các flow legacy còn lại theo nhóm: (1) money subflows: budget, partner sponsorship/fee/gift/benefit, supplier quote/transaction/contact mutation; (2) monitor highlights/competitor brief/alerts và các báo cáo AI. | **Partner financial + supplier direct mutations và monitor runs/competitors/campaigns/settings đã có Desktop/Native local composition, API allowlist, permission gate và unit contract.** Budget đã có slice; AI highlights/brief và AI card text/image được owner hoãn. | Không gọi là “100% native” trước khi từng flow có route/surface/role evidence; ưu tiên theo giá trị nghiệp vụ và dữ liệu nhạy cảm, không theo số dòng. |
| P1 | Hoàn chỉnh điều hướng native giữa những slice đã có và danh sách/entry point của các slice mới. | Supplier list đã có native resolver/taskbar entry; các flow mới còn lại cần giữ invariant này khi tách. | Không có navigation native nào rơi về legacy desktop; test route transition, Back, 403 và safe-area cho mỗi entry point. |
| CLOSED | `relationship_score` AI policy / W3.VOICE.0 | Owner 2026-09-02 chốt score nhập tay; server/UI Voice đã bỏ proposal/delta/write, kể cả input tamper/proposal cũ. | Chỉ mở lại như một feature BA mới có policy và test riêng. |
| P2 | Chốt audit UI cuối sau khi backlog F6 của release scope đã xong. | Dashboard, Partner List/Create và Supplier List/Create nay có test/build (Dashboard/Partner đã browser fake-native); vẫn chưa có một evidence matrix cuối bao phủ mọi flow mới × Desktop/Native × 4 role. | Rà ma trận thực tế, không dùng baseline `N-MISSING` Gate 0 để tự gọi PASS; chỉ review những slice mới/chưa audited, không audit lại backend đã CLOSED. |

**Không thuộc backlog repository:** O3/O4/O5 và W4.1–W4.4/W4.VOICE thuộc DevOps/AMIS; O8 thuộc
Security/Legal. Những việc này đã được note trong `32-production-handoff.md` và phải được làm trong
môi trường MISA khi nhận build, không tự triển khai/suy đoán trong codebase này.
...
> **Execution update — 2026-09-02 (W2.2/F6 — Monitoring operations Desktop + Native):** tách `monitor/operations` thành một vertical slice dùng chung API/domain cho R122–R135: lịch sử quét, chạy scan theo query đang bật, competitor CRUD, campaign CRUD/results/evaluation và settings. Desktop dùng card/table MDS; Native là mini-app `.mds-mobile-app` với top bar, safe-area, touch target 48px và composition riêng, không fallback về desktop. `UI-MONITOR-006/007/008` khóa projection, payload allowlist, API method/path và permission affordance; local native route đã kiểm tra trực quan. Chạy scan luôn nạp lại query và chỉ gửi ID query đang enabled; `/api/me` được nạp cho deep-link trực tiếp để không mất quyền thao tác. AI evaluation vẫn là action có chủ đích, không tự ghi dữ liệu. Còn AI highlights/competitor brief/card là owner-deferred, không dựng UI giả.

**Exit gate W2:** shared layer swap được qua contract test; **W2.3 PASS** với profile MySQL 50 người dùng tác nghiệp không lỗi theo D17 (không diễn giải thành SLO enterprise); W2.5 contract-ready (2 fake provider); **W2.6 XONG — kết luận GIỮ PIN `gemini-3.5-flash`** (candidate `gemini-3.7-flash` không thắng rõ, trong ngân sách $4.456/$200).
