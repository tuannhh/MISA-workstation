...
test('UI-CHAR-003: ma trận hiện trạng phủ 34 flow/145 route và ghi Native-Mobile là missing cho hai role', () => {
  const flowRows = tableRows(matrix, 'F\\d{3}\\s*\\|');
  assert.equal(flowRows.length, 34, 'G1A.6 phải kiểm soát toàn bộ 34 business/UI flow, không chỉ nav');

  const routeIds = flowRows.flatMap((line) => [...line.matchAll(/R\d{3}/g)].map((match) => match[0]));
  assert.equal(routeIds.length, 145, '34 flow phải ánh xạ đủ 145 route');
