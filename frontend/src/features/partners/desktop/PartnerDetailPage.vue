...
const tabs = [{ key: 'overview', label: 'Thông tin chung' }, { key: 'people', label: 'Nhân sự' }, { key: 'dates', label: 'Ngày nhắc' }, { key: 'cooperation', label: 'Hợp tác' }];
</script>
  ...
  <section class="min-h-0 bg-[var(--mds-bg-page)] p-4">
    ...
    <div class="rounded-lg bg-[var(--mds-bg)] shadow-[var(--mds-shadow-card)]">
      <MTabs v-model="activeTab" :tabs="tabs" class="[&>[role=tablist]]:px-4">
        ...
        <section v-else-if="activeTab === 'dates'" class="p-4"><h2 class="text-[16px] font-semibold leading-[22px]">Ngày nhắc liên quan</h2><div v-if="detail.dates.length" class="mt-3 divide-y divide-[var(--mds-border-light)]"><div v-for="date in detail.dates" :key="date.id" class="flex min-h-14 items-center justify-between gap-4 py-3"><span class="min-w-0"><strong class="block truncate text-[13px]">{{ date.title }}</strong><span v-if="date.recurring" class="mt-1 block text-[12px] text-[var(--mds-text-secondary)]">Lặp lại hằng năm</span></span><span class="shrink-0 text-[13px] font-medium">{{ date.date }}</span></div></div><MEmptyState v-else title="Chưa có ngày nhắc" description="Ngày thành lập và các mốc hợp tác sẽ xuất hiện tại đây khi được tạo." /></section>
        <section v-else class="grid gap-6 p-4 lg:grid-cols-2"><div><h2 class="text-[16px] font-semibold leading-[22px]">Thỏa thuận / MOU</h2><div v-if="detail.agreements.length" class="mt-3 divide-y divide-[var(--mds-border-light)]"><div v-for="agreement in detail.agreements" :key="agreement.id" class="py-3"><strong class="block text-[13px]">{{ agreement.title }}</strong><span class="mt-1 block text-[12px] text-[var(--mds-text-secondary)]">Ký {{ agreement.signedDate }} · Hiệu lực đến {{ agreement.validUntil }}</span></div></div><MEmptyState v-else title="Chưa có thỏa thuận" description="Thỏa thuận được quản lý trong slice Partner File/MOU riêng." /></div><div><h2 class="text-[16px] font-semibold leading-[22px]">Lịch sử làm việc</h2><div v-if="detail.workLogs.length" class="mt-3 divide-y divide-[var(--mds-border-light)]"><div v-for="work in detail.workLogs" :key="work.id" class="py-3"><strong class="block text-[13px]">{{ work.title }}</strong><span class="mt-1 block text-[12px] text-[var(--mds-text-secondary)]">{{ work.category }} · {{ work.date }} · {{ work.status }}</span></div></div><MEmptyState v-else title="Chưa có lịch sử làm việc" description="Lịch sử làm việc được quản lý trong slice Partner File/MOU riêng." /></div></section>
      </MTabs>
    </div>
