...
async function beginOwnerReassign(entity, record) { if (!canReassignCooperation.value(record)) return; state.ownerReassignError = ''; state.ownerReassignTarget = null; try { state.ownerReassign = ownerReassignViewModel({ record: { owner_id: record.ownerId }, users: await api.getAdminUsers() }); state.ownerReassignContext = { entity, id: Number(record.id), title: record.title, resourceLabel: entity === 'agreement' ? 'Thỏa thuận / MOU' : 'Lịch sử làm việc' }; state.ownerId = state.ownerReassign.ownerId; state.mode = 'owner-reassign'; } catch (error) { state.ownerReassignError = error.message; state.ownerReassignContext = { entity, id: Number(record?.id), title: record?.title, resourceLabel: entity === 'agreement' ? 'Thỏa thuận / MOU' : 'Lịch sử làm việc' }; state.mode = 'owner-reassign'; } }
