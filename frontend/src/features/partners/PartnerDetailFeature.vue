...
import PartnerFinancialFormDesktop from './desktop/PartnerFinancialFormDesktop.vue';
...
import PartnerFinancialFormMobile from './mobile/PartnerFinancialFormMobile.vue';
...
const state = reactive({ phase: 'loading', detail: null, record: null, currentUser: null, permissions: null, editingAgreement: null, editingWorkLog: null, financialEntity: null, financialRecord: null, financialDeleteTarget: null, ownerReassign: null, ownerReassignContext: null, ownerId: null, ownerReassignTarget: null, ownerReassignSaving: false, ownerReassignError: '', error: null, mode: 'read', saving: false, saveError: null, deleting: false, deleteError: null, deletingCooperationId: null, cooperationDeleteError: null, uploadingCooperationId: null, fileError: null });
...
const canCreateFinancial = computed(() => state.permissions?.modules?.partners?.includes('create') === true);
const canDeleteFinancial = computed(() => state.permissions?.modules?.partners?.includes('delete') === true && ['admin', 'super_admin'].includes(state.currentUser?.role));
const canSeeFinancialMoney = computed(() => ['admin', 'super_admin'].includes(state.currentUser?.role));
function canEditFinancial(item) { return state.permissions?.modules?.partners?.includes('edit') === true && (['admin', 'super_admin'].includes(state.currentUser?.role) || (state.currentUser?.role === 'executor' && Number(item?.ownerId) === Number(state.currentUser?.id))); }
...
function beginFinancial(entity, record = null) { if (record ? !canEditFinancial(record) : !canCreateFinancial.value) return; state.financialEntity = entity; state.financialRecord = record; state.saveError = null; state.mode = 'financial-form'; }
async function saveFinancial(payload) { if (!state.financialEntity || (state.financialRecord && !canEditFinancial(state.financialRecord)) || (!state.financialRecord && !canCreateFinancial.value)) return; state.saving = true; state.saveError = null; try { await api.saveFinancial(props.partnerId, state.financialEntity, payload, state.financialRecord?.id); await load(); } catch (error) { state.saveError = error instanceof PartnerApiError ? error.message : 'Không thể lưu khoản cơ quan. Vui lòng thử lại.'; } finally { state.saving = false; } }
function askDeleteFinancial({ entity, record }) { if (canDeleteFinancial.value && record?.id) state.financialDeleteTarget = { entity, record }; }
async function deleteFinancial() { const target = state.financialDeleteTarget; if (!canDeleteFinancial.value || !target?.record?.id) return; state.financialDeleteTarget = null; state.cooperationDeleteError = null; try { await api.deleteFinancialForPartner(props.partnerId, target.entity, target.record.id); await load(); } catch (error) { state.cooperationDeleteError = error instanceof PartnerApiError ? error.message : 'Không thể xóa khoản cơ quan. Vui lòng thử lại.'; } }
...
<template>
  ...
  <PartnerFinancialFormMobile v-else-if="isNative && state.mode === 'financial-form'" :entity="state.financialEntity" :record="state.financialRecord" :can-see-money="canSeeFinancialMoney" :saving="state.saving" :error="state.saveError" :safe-area-style="safeAreaStyle" @back="state.mode='read'" @save="saveFinancial" @local-error="state.saveError=$event" />
  <PartnerDetailPageMobile v-else-if="isNative" :detail="state.detail" :can-edit="canEdit" :can-delete="canDelete" :can-create-cooperation="canCreateCooperation" :can-edit-cooperation="canEditCooperation" :can-create-financial="canCreateFinancial" :can-edit-financial="canEditFinancial" :can-delete-financial="canDeleteFinancial" :can-reassign-cooperation="canReassignCooperation" :can-upload-cooperation="canEditCooperation" :can-delete-cooperation="canDelete" :deleting-cooperation-id="state.deletingCooperationId" :uploading-cooperation-id="state.uploadingCooperationId" :cooperation-delete-error="state.cooperationDeleteError" :file-error="state.fileError" :delete-working="state.deleting" :delete-error="state.deleteError" :safe-area-style="safeAreaStyle" @back="goBack" @edit="state.mode = 'edit'" @create-agreement="state.mode = 'agreement-create'" @create-work-log="state.mode = 'work-log-create'" @edit-agreement="beginAgreementEdit" @edit-work-log="beginWorkLogEdit" @create-financial="beginFinancial($event)" @edit-financial="beginFinancial($event.entity, $event.record)" @delete-financial="askDeleteFinancial" @reassign-agreement="beginOwnerReassign('agreement', $event)" @reassign-work-log="beginOwnerReassign('work_log', $event)" @reassign-direct="beginOwnerReassign($event.entity, $event.record)" @upload-agreement="uploadAgreementFiles" @upload-work-log="uploadWorkLogFiles" @delete-agreement="deleteAgreement" @delete-work-log="deleteWorkLog" @delete-partner="deletePartner" @navigate="emit('navigate', $event)" />
  ...
  <PartnerFinancialFormDesktop v-else-if="state.mode === 'financial-form'" :entity="state.financialEntity" :record="state.financialRecord" :can-see-money="canSeeFinancialMoney" :saving="state.saving" :error="state.saveError" @back="state.mode='read'" @save="saveFinancial" @local-error="state.saveError=$event" />
  ...
  <MDialog :model-value="Boolean(state.financialDeleteTarget)" title="Xóa khoản cơ quan?" type="danger" confirm-text="Xóa" @update:model-value="!$event&&(state.financialDeleteTarget=null)" @confirm="deleteFinancial"><p>“{{ state.financialDeleteTarget?.record?.title }}” sẽ bị xóa. Máy chủ kiểm tra lại quyền trước khi thực hiện.</p></MDialog>
</template>
