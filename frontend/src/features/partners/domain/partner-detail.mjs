...
export function partnerDetailViewModel(payload) {
  ...
  const sponsorships = Array.isArray(payload?.sponsorships) ? payload.sponsorships.map((record) => Object.freeze({
    id: Number(record.id), ownerId: Number.isInteger(Number(record.owner_id)) ? Number(record.owner_id) : null,
    title: text(record.title), subtitle: [record.type, formatDate(record.event_date), record.status].filter(Boolean).join(' · ') || 'Chưa cập nhật',
  })) : [];
  const gifts = Array.isArray(payload?.gifts) ? payload.gifts.map((record) => Object.freeze({
    id: Number(record.id), ownerId: Number.isInteger(Number(record.responsible_user_id)) ? Number(record.responsible_user_id) : null,
    title: [record.gift_type, record.occasion].filter(Boolean).join(' · ') || 'Quà tặng', subtitle: [record.giver, formatDate(record.event_date)].filter(Boolean).join(' · ') || 'Chưa cập nhật',
  })) : [];
  const associationFees = Array.isArray(payload?.fees) ? payload.fees.map((record) => Object.freeze({
    id: Number(record.id), ownerId: Number.isInteger(Number(record.owner_id)) ? Number(record.owner_id) : null,
    title: `Hội phí ${text(record.year)}`, subtitle: [record.status, formatDate(record.due_date)].filter(Boolean).join(' · ') || 'Chưa cập nhật',
  })) : [];
  const benefitUsages = Array.isArray(payload?.benefitUsages) ? payload.benefitUsages.map((record) => Object.freeze({
    id: Number(record.id), ownerId: Number.isInteger(Number(record.owner_id)) ? Number(record.owner_id) : null,
    title: text(record.title), subtitle: formatDate(record.used_date) || 'Chưa cập nhật ngày sử dụng',
  })) : [];
