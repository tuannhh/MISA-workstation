...
const state = reactive({ phase: 'loading', rows: [], total: 0, page: 1, pageSize: 20, search: '', detail: null, record: null, participationRecords: [], participationRecord: null, permissions: null, userId: null, mode: 'read', saving: false, saveError: '', participationError: '', error: null });
...
<template><div v-if="state.phase === 'loading'" :class="isNative ? 'mds-mobile-app grid h-[100dvh] place-items-center bg-[var(--mds-bg)]' : 'grid min-h-[360px] place-items-center bg-[var(--mds-bg-page)]'"><MSpinner :size="28"/></div><section v-else-if="state.phase === 'error'" :class="isNative ? 'mds-mobile-app min-h-[100dvh] bg-[var(--mds-bg)]' : 'min-h-[360px] bg-[var(--mds-bg-page)]'"><MMobileTopBar v-if="isNative" title="Giải thưởng" @back="back"/><MEmptyState :title="errorTitle" :description="state.error.message"/></section><AwardParticipationFormMobile v-else-if="isNative && ['participation-create', 'participation-edit'].includes(state.mode)" :record="state.participationRecord" :editing="state.mode === 'participation-edit'" :saving="state.saving" :error="state.participationError" :safe-area-style="safeAreaStyle" @cancel="cancelParticipation" @save="saveParticipation"/><AwardFormMobile v-else-if="isNative && ['create', 'edit'].includes(state.mode)" :record="state.record" :editing="state.mode === 'edit'" :saving="state.saving" :error="state.saveError" :safe-area-style="safeAreaStyle" @cancel="state.mode = props.awardId ? 'detail' : 'read'" @save="save"/><AwardDetailMobile v-else-if="isNative && state.mode === 'detail'" :detail="state.detail" :can-edit="canEdit" :can-create-participation="canCreateParticipation" :can-edit-participation="canEditParticipation" :can-delete-participation="canDeleteParticipation" :participation-error="state.participationError" :safe-area-style="safeAreaStyle" @back="list" @edit="edit" @add-participation="beginParticipation" @edit-participation="editParticipation" @delete-participation="deleteParticipation"/><AwardsListMobile v-else-if="isNative" :rows="state.rows" :total="state.total" :page="state.page" :page-size="state.pageSize" :search="state.search" :can-create="canCreate" :safe-area-style="safeAreaStyle" @back="back" @create="create" @open="open" @page="load({ page: $event })" @search="load({ page: 1, search: $event })"/><AwardParticipationFormDesktop v-else-if="['participation-create', 'participation-edit'].includes(state.mode)" :record="state.participationRecord" :editing="state.mode === 'participation-edit'" :saving="state.saving" :error="state.participationError" @cancel="cancelParticipation" @save="saveParticipation"/><AwardFormDesktop v-else-if="['create', 'edit'].includes(state.mode)" :record="state.record" :editing="state.mode === 'edit'" :saving="state.saving" :error="state.saveError" @cancel="state.mode = props.awardId ? 'detail' : 'read'" @save="save"/><AwardDetailDesktop v-else-if="state.mode === 'detail'" :detail="state.detail" :can-edit="canEdit" :can-create-participation="canCreateParticipation" :can-edit-participation="canEditParticipation" :can-delete-participation="canDeleteParticipation" :participation-error="state.participationError" @back="list" @edit="edit" @add-participation="beginParticipation" @edit-participation="editParticipation" @delete-participation="deleteParticipation"/><AwardsListDesktop v-else :rows="state.rows" :total="state.total" :page="state.page" :page-size="state.pageSize" :search="state.search" :loading="state.phase === 'loading'" :can-create="canCreate" @create="create" @open="open" @page="load({ page: $event })" @search="load({ page: 1, search: $event })"/></template>
