...
const emit = defineEmits(['search', 'page', 'create', 'back']);
...
<template>
  <section class="mds-mobile-app flex h-[100dvh] min-w-0 flex-col overflow-hidden bg-[var(--mds-bg)]" :style="safeAreaStyle">
    <MMobileTopBar title="Lịch sử tương tác" @back="emit('back')"><template #actions><MButton v-if="canCreate" variant="link" :disabled="loading" @click="emit('create')">Thêm</MButton></template></MMobileTopBar>
