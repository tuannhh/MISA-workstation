...
defineProps({ detail: { type: Object, required: true }, canEdit: Boolean, canDelete: Boolean, canManageCosts: Boolean, costDeleting: Boolean, costError: String, deleteWorking: Boolean, deleteError: String, uploading: Boolean, fileError: String });
const emit = defineEmits(['back', 'edit', 'delete', 'upload', 'add-cost', 'edit-cost', 'delete-cost']); const deleteOpen = ref(false);
</script>
  ...
  <section class="min-h-0 bg-[var(--mds-bg-page)] p-4">
    <div class="mx-auto max-w-[1200px] space-y-4">
      <header class="flex min-h-14 items-start justify-between gap-4 rounded-lg bg-[var(--mds-bg)] px-5 py-4 shadow-[var(--mds-shadow-card)]">
        ...
        <div class="flex shrink-0 items-center gap-2"><MButton variant="neutral" @click="emit('back')">Quay lại</MButton><MButton v-if="canDelete" variant="danger" :disabled="deleteWorking" @click="deleteOpen = true">Xóa</MButton><MButton v-if="canEdit" variant="primary" @click="emit('edit')">Sửa</MButton><MTag color="neutral">{{ detail.status }}</MTag></div>
      </header>
