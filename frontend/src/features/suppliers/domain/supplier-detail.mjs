...
export function supplierDetailViewModel(payload) {
  ...
  const quotes = directRows(payload?.quotes, (quote) => Object.freeze({ id: Number(quote.id), ownerId: Number.isInteger(Number(quote.owner_id)) ? Number(quote.owner_id) : null, item: text(quote.item), unit: text(quote.unit), qty: text(quote.qty), stt: quote.stt ?? '', hasUnitPrice: Object.prototype.hasOwnProperty.call(quote, 'unit_price'), unitPrice: quote.unit_price, ...(Object.prototype.hasOwnProperty.call(quote, 'unit_price') ? { unit_price: quote.unit_price } : {}) }));
  const transactions = directRows(payload?.transactions, (transaction) => Object.freeze({ id: Number(transaction.id), ownerId: Number.isInteger(Number(transaction.owner_id)) ? Number(transaction.owner_id) : null, title: text(transaction.contract_no || transaction.purpose || transaction.service_type), serviceType: text(transaction.service_type), purpose: text(transaction.purpose), status: text(transaction.status), signedDate: date(transaction.signed_date), deadline: date(transaction.exec_deadline), hasValue: Object.prototype.hasOwnProperty.call(transaction, 'value'), value: transaction.value, service_type: transaction.service_type || '', purpose: transaction.purpose || '', contract_no: transaction.contract_no || '', signed_date: transaction.signed_date || '', exec_deadline: transaction.exec_deadline || '', status: transaction.status || '', staff: transaction.staff || '', note: transaction.note || '', ...(Object.prototype.hasOwnProperty.call(transaction, 'value') ? { value: transaction.value } : {}) }));
  const contacts = directRows(payload?.contacts, (contact) => Object.freeze({ id: Number(contact.id), ownerId: Number.isInteger(Number(contact.owner_id)) ? Number(contact.owner_id) : null, name: text(contact.full_name), position: text(contact.position), role: text(contact.role), phone: text(contact.phone), email: text(contact.email), full_name: contact.full_name || '', position: contact.position || '', phone: contact.phone || '', email: contact.email || '', role: contact.role || '', note: contact.note || '' }));
