...
const state = reactive({ phase: 'loading', rows: [], total: 0, page: 1, pageSize: 20, search: '', error: null }); const safeArea = ref(props.adapter?.getSafeArea?.() || {}); const unsubscribers = [];
...
async function load({ page = state.page, search = state.search } = {}) { if (props.hostUnavailable) { setError(new PeopleApiError({ status: 503, code: 'HOST_ADAPTER_UNAVAILABLE', message: 'Native host chưa sẵn sàng. Ứng dụng không chuyển sang giao diện desktop thay thế.' })); return; } state.phase = 'loading'; state.error = null; try { const model = peopleListViewModel(await api.getList({ page, pageSize: state.pageSize, search })); state.rows = model.rows; state.total = model.total; state.page = model.page; state.pageSize = model.pageSize; state.search = search; state.phase = 'ready'; } catch (error) { setError(error); } }
