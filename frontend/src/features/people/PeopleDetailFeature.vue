...
async function load() {
  ...
  try { state.detail = peopleDetailViewModel(await api.getDetail(props.personId)); state.phase = 'ready'; } catch (error) { setError(error); }
}
...
<template>
  ...
  <PeopleDetailPage v-else :detail="state.detail" @back="goBack" />
</template>
